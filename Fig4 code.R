...
analyze_set <- function(isnv_file, callable_file, set_name) {
  ...
  m2 <- purrr::map_dfr(genes_of_interest, function(g) {
    ...
    tryCatch({
      ...
      # S-slope (= log10(Dose)+interaction) self p-value; SE matches IRR_S CI
      se_S <- sqrt(sd_d^2 + si^2)
      p_S  <- 2*pnorm(-abs((cd+ci)/se_S))
      data.frame(Gene=g,
                 ...
                 Sig_S=ifelse(is.na(p_S),"NA",ifelse(p_S<0.001,"***",ifelse(p_S<0.01,"**",ifelse(p_S<0.05,"*","ns")))),
